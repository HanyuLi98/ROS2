...
class ParamServer(Node):
    ...
    def del_param(self):
        ...
        self.get_logger().info("删除前包含 car_name 吗 %d" % self.has_parameter("car_name"))
        self.undeclare_parameter("car_name")
        self.get_logger().info("删除后包含 car_name 吗 %d" % self.has_parameter("car_name"))
def main():
    #2.初始化ROS2客户端
    rclpy.init()
    #4.调用spin函数，并传入节点对象
